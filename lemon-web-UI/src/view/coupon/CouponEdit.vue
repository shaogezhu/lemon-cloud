    ...
    <div class="wrap">
      <el-row>
        <el-col :lg="16" :md="20" :sm="24" :xs="24">
          <el-form :model="form" status-icon ref="form" label-width="100px" @submit.enter.prevent>
...
<!--               <el-input size="medium" v-model="form.type" disabled></el-input>-->
              <el-select v-model="form.type" placeholder="请填写类型" disabled>
            ...
            <el-form-item label="选择分类" prop="category_ids" v-if="form.type!==3">
              <el-cascader
                placeholder="选择分类"
            ...
            <el-form-item label="使用范围" prop="escription">
              <el-input size="medium" v-model="form.remark" placeholder="请填写优惠卷使用范围提示"></el-input>
            </el-form-item>
...
export default {
  ...
  async mounted() {
    ...
    if (!this.isCreate) {
      ...
      this.category_ids = res.category_ids
    } else {
  ...
  methods: {
    async submitForm(formName) {
      ...
      try {
        ...
        if (postData.type === 3) {
          postData.category_ids = null
        } else {
          postData.category_ids = this.formatCategoryIds(postData.category_ids)
        }
