<template>
  <div>
    <div v-if="!showEdit" class="container">
      ...
      <el-table stripe v-loading="loading" :data="tableData" max-height="2000px">
        <el-table-column prop="id" label="id" width="100"></el-table-column>
        <el-table-column prop="avatar" label="头像" width="200">
          <template v-slot="scope">
            <img v-if="scope.row.avatar" class="display_img" :src="scope.row.avatar" :alt="scope.row.avatar" />
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" prop="openid" label="openid" width="300">
          <template v-slot="scope"
            ><span>{{ scope.row.openid }}</span></template
          >
        </el-table-column>
