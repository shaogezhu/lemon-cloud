      ...
      <el-table stripe v-loading="loading" :data="tableData">
        ...
        <el-table-column prop="img" width="100" label="图片">
          <template v-slot="scope">
            <el-image v-if="scope.row.img" :src="scope.row.img" :preview-src-list="imgSrcList" style="max-width: 200px">
            </el-image>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="online" label="是否上架" width="150">
          <template v-slot="scope"
            ><span v-if="scope.row.online">{{ onLine(scope.row.online) }}</span></template
          >
        </el-table-column>
